...
function createScene() {
  ...
  scene.fog = new THREE.Fog(0x0ad4ff , 100,950);
  camera.position.x = 0;
...
function createLights() {
  ...
  ambientLight = new THREE.AmbientLight(0x5E1DFF, .5);
...
Ennemy = function(){
  var geom = new THREE.TetrahedronGeometry(9,3);
  var mat = new THREE.MeshPhongMaterial({
    color:Colors.black,
    shininess:0,
    specular:0xffffff,
...
EnnemiesHolder.prototype.rotateEnnemies = function(){
  for (var i=0; i<this.ennemiesInUse.length; i++){
    ...
    if (d<game.ennemyDistanceTolerance){
      particlesHolder.spawnParticles(ennemy.mesh.position.clone(), 15, Colors.black, 3);
